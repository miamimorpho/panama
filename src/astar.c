...
siftToLeaf(struct AStar *a, struct AStarNode *n)
{
	...
	do {
		...
		if (l < a->heap_top && fScore(left) < fScore(current)) {
			smallest = l;
		}

		if (r < a->heap_top && fScore(right) < fScore(current)) {
			smallest = r;
		}
...
int
aStarBuildPath(struct AStar *a, vec16 *path, uint32_t *path_len)
{
	...
	if (!path) {
		*path_len = 0;
		...
		vec16 cur;
		for (vec16Copy(a->goal, cur); !vec16Equal(cur, a->start);
...
static void
getNesw_(struct Terrain *tm, vec16 tmp, vec16 nesw[4], uint32_t *c)
{
	struct TerraPos p;
	p = terraPos(tm, tmp);
	if (!terraGetSolid(p)) {
...
static void
getNesw(struct Terrain *tm, vec16 o, vec16 nesw[4], uint32_t *c)
{
	getNesw_(tm, VEC16_NORTH(o), nesw, c);
	getNesw_(tm, VEC16_EAST(o), nesw, c);
	getNesw_(tm, VEC16_SOUTH(o), nesw, c);
	getNesw_(tm, VEC16_WEST(o), nesw, c);
}

int
aStar(struct Dungeon *d, vec16 start, vec16 goal, struct AStar **aa)
{
	...
	first.h = manhattanDist(start, goal), vec16Copy(start, first.pos);
	...
	while (nodePop(a, &current) == 0) {
		...
		vec16 nesw[4];
		uint32_t nesw_c = 0;
		getNesw(d->terrain, current.pos, nesw, &nesw_c);
		...
		for (uint32_t i = 0; i < nesw_c; i++) {
			...
			if (neighbor->state == OPEN) {
				double g =
					current.g + manhattanDist(current.pos, neighbor->pos);
				if (g < neighbor->g) {
