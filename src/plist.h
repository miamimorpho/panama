...
#define PLIST_FOREACH(pool, head, tmp, field)                                  \
	for ((tmp) = PLIST_FIRST(pool, head); (tmp) != PLIST_NULL;                 \
		 (tmp) = PLIST_NEXT(pool, tmp, field))

#define PLIST_INSERT_HEAD(pool, head, elm, field)                              \
	do {                                                                       \
		(elm)->field.sle_next = (head)->slh_first;                             \
		(head)->slh_first = PLIST_PTR(pool, elm);                              \
	} while (0)

#define PLIST_REMOVE_HEAD(pool, head, field)                                   \
	do {                                                                       \
		if ((head)->slh_first) {                                               \
...
#define PLIST_REMOVE(pool, head, elm, field)                                   \
	do {                                                                       \
