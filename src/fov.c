...
static void
shadowToWorldPos(enum Cardinal dir, uint32_t depth, uint32_t col, vec16 cam, vec16 out)
{
    switch(dir) {
        case NORTH:
            out[0] = cam[0] + col;
...
// fast, approximate
static inline int frCompare(Fraction a, Fraction b) {
...
static void
scanRow(struct Row cur, struct Dungeon *d, float depth_max, struct FovEffect *effect){
    ...
    for(int col = min; col <= max; col++){
        ...
        struct TerraPos ter = terraPos(d, pos, 0);
...
fov(struct Dungeon *d, vec16 o, struct FovEffect *effect){
    struct TerraPos ter = terraPos(d, o, 0);
